...
if not sheets:
    ...
    if go:
        ...
        with st.spinner("Building dashboard..."):
            ...
            # Prefer dispatch sheet for hist — it has real invoice/payment data (inv_value, pay_status, pay_due).
            # Fall back to items sheet only when dispatch sheet is absent.
            if "dispatch" in sheets:
                hist = build_dispatch_history(sheets.get("dispatch", pd.DataFrame()), selected_customer, today_ts)
            elif "items" in sheets:
                hist = build_hist_from_items(sheets.get("items", pd.DataFrame()), selected_customer)
            else:
                hist = []
